...
#define __fseeko(fstream, offset, whence) fseeko(fstream, offset, whence)
#define __ftello(stream) ftello(stream)
#else
#define likely(x) x
#define unlikely(x) x
#define __fseeko(fstream, offset, whence) _fseeki64(fstream, offset, whence)
#define __ftello(stream) _ftelli64(stream)
#endif
...
#define SSDEEP_BS(index) (((uint64_t)MIN_BLOCKSIZE) << (index))
...
/*@only@*/ /*@null@*/ struct fuzzy_state* fuzzy_new(void)
{
    ...
    if (NULL == (self = static_cast<fuzzy_state*>(malloc(sizeof(struct fuzzy_state)))))
        /* malloc sets ENOMEM */
        return NULL;
    self->bhstart = 0;
    self->bhend = 1;
...
/*@only@*/ /*@null@*/ struct fuzzy_state* fuzzy_clone(const struct fuzzy_state* state)
{
    ...
    if (NULL == (newstate = static_cast<fuzzy_state*>(malloc(sizeof(struct fuzzy_state)))))
        /* malloc sets ENOMEM */
        return NULL;
...
int fuzzy_update(struct fuzzy_state* self, const unsigned char* buffer, uint64_t buffer_size)
{
...
int fuzzy_digest(const struct fuzzy_state* self, /*@out@*/ char* result, unsigned int flags)
{
...
int fuzzy_hash_buf(const unsigned char* buf, uint64_t buf_len, /*@out@*/ char* result)
{
...
int fuzzy_hash_file(FILE* handle, /*@out@*/ char* result)
{
    ...
    int status = -1;
    struct fuzzy_state* ctx;
    fpos = static_cast<off_t>(__ftello(handle));
    if (fpos < 0)
        return -1;
    if (__fseeko(handle, 0, SEEK_END) < 0)
        return -1;
    fposend = static_cast<off_t>(__ftello(handle));
    if (fposend < 0)
        return -1;
    if (__fseeko(handle, 0, SEEK_SET) < 0)
        return -1;
    if (NULL == (ctx = fuzzy_new()))
        return -1;
...
out:
    if (status == 0)
    {
        if (__fseeko(handle, fpos, SEEK_SET) < 0)
            status = -1;
    }
    fuzzy_free(ctx);
    return status;
...
// messages.
//
static uint32_t score_strings(const char* s1, size_t s1len, const char* s2, size_t s2len, unsigned long block_size)
{
    uint32_t score;
    ...
    // the string lengths.
    score = static_cast<uint32_t>((score * SPAMSUM_LENGTH) / (s1len + s2len));
    ...
    {
        score = static_cast<uint32_t>(block_size / MIN_BLOCKSIZE * MIN(s1len, s2len));
    }
    return score;
}
...
int fuzzy_compare(const char* str1, const char* str2)
{
    ...
    if (block_size1 <= ULONG_MAX / 2) {
        if (block_size1 == block_size2) {
            uint32_t score1 = score_strings(s1b1, s1b1len, s2b1, s2b1len, block_size1);
            uint32_t score2 = score_strings(s1b2, s1b2len, s2b2, s2b2len, block_size1 * 2);

...
struct IMAGE_OPTIONAL_HEADER32 {
    WORD        Magic;              // Identifies PE32 or PE32+ format
...
struct IMAGE_NT_HEADERS32 {
    DWORD Signature; // 'PE\0\0' signature (0x00004550)
    IMAGE_FILE_HEADER FileHeader;
    IMAGE_OPTIONAL_HEADER32 OptionalHeader;
};
...
struct IMAGE_SECTION_HEADER {
    BYTE  Name[8]; // Name of the section
    union {
        DWORD PhysicalAddress;
        DWORD VirtualSize;
    } Misc;
    DWORD VirtualAddress;   // RVA of the section
...
struct IMAGE_IMPORT_DESCRIPTOR {
    union {
        DWORD Characteristics;    // RVA to original unbound IAT
        DWORD OriginalFirstThunk; // Same as Characteristics
    };
    DWORD TimeDateStamp;          // Time/date stamp of the import
    DWORD ForwarderChain;
    DWORD Name;                   // RVA to the module name
...
struct IMAGE_IMPORT_BY_NAME {
    WORD Hint;          // Index into the export name table
    BYTE Name[1];       // Null-terminated string
};
...
struct IMAGE_THUNK_DATA32 {
    union {
        ...
        DWORD Function;         // RVA to function
...
struct IMAGE_THUNK_DATA64 {
    union {
        ULONGLONG ForwarderString;  // RVA to forwarder string
        ULONGLONG Function;         // RVA to function
...
struct IMAGE_BASE_RELOCATION {
    DWORD   VirtualAddress; // RVA of the base relocation block
    DWORD   SizeOfBlock;    // Size of the block, including entries
};
